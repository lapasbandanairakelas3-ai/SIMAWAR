...
// ── INIT ──────────────────────────────────────────────────────
async function init() {
  // Hapus splash setelah 900ms — tidak tunggu data
  setTimeout(() => {
...
// ── NAVIGASI ──────────────────────────────────────────────────
// Nav item punya id="nav-{page}" — simple dan tidak ambigu
function goPage(page) {
  ...
  // Active nav — pakai id
  const navIds = ['dashboard','wbp','pegawai','blok','riwayat','siteconfig','akun'];
  navIds.forEach(id => {
    const el = document.getElementById('nav-' + id);
    if (el) el.classList.toggle('active', id === page);
  });

  const titles = { dashboard:'Dashboard', wbp:'Data WBP', pegawai:'Data Pegawai', blok:'Blok / Kamar', riwayat:'Riwayat Absensi', siteconfig:'Konfigurasi Website', akun:'Akun Saya' };
...
async function loadSiteConfigForm() {
  ...
  v('cfgName', data.site_name); v('cfgDesc', data.site_desc); v('cfgGas', data.gas_url);
  v('cfgInstansi', data.instansi); v('cfgAlamat', data.alamat);
  if (data.logo_url) {
...
function previewLogo(e) {
  ...
  const r = new FileReader(); r.onload = ev => {
    const img = document.getElementById('logoPreview'); if (img) { img.src = ev.target.result; img.style.display = 'block'; }
    const ph = document.getElementById('logoPlaceholder'); if (ph) ph.style.display = 'none';
    const rb = document.getElementById('removeLogoBtn'); if (rb) rb.style.display = 'inline-flex';
...
function previewFav(e) {
  ...
  const r = new FileReader(); r.onload = ev => {
    const img = document.getElementById('favPreview'); if (img) { img.src = ev.target.result; img.style.display = 'block'; }
    const ph = document.getElementById('favPlaceholder'); if (ph) ph.style.display = 'none';
...
async function testGasConnection() {
  ...
  } catch { if (el) { el.className='p-3 rounded-xl text-xs mb-3 bg-red-50 text-red-700'; el.textContent='❌ Gagal. Cek URL dan pastikan deploy sebagai Anyone.'; } }
}
async function resetSiteConfig() {
  showConfirm('Reset','Yakin reset konfigurasi ke default?', async () => {
    ...
    showAlert('success','Reset!','Konfigurasi direset'); loadSiteConfig(); loadSiteConfigForm();
  },'danger');
}
...
async function saveSiteConfig() {
  ...
  try {
    if (logoFile) payload.logo_url = await uploadToSupabase(logoFile,'assets',`logo_${Date.now()}.${logoFile.name.split('.').pop()}`);
    if (favFile)  payload.favicon_url = await uploadToSupabase(favFile,'assets',`fav_${Date.now()}.${favFile.name.split('.').pop()}`);
    ...
    logoFile = null; favFile = null; loadSiteConfig();
...
async function loadDashboard() {
  ...
  document.getElementById('dashStats').innerHTML = [
    {label:'Total WBP',val:wR.count||0,bg:'bg-blue-50',clr:'text-blue-500',icon:'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z'},
    {label:'Petugas Aktif',val:pR.count||0,bg:'bg-green-50',clr:'text-green-500',icon:'M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6a4 4 0 11-8 0 4 4 0 018 0zm-4 9a9 9 0 00-9 9h18a9 9 0 00-9-9z'},
    {label:'Blok / Kamar',val:bR.count||0,bg:'bg-amber-50',clr:'text-amber-500',icon:'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6'},
    {label:'Absensi Hari Ini',val:'<span id="dashAbsenVal">—</span>',bg:'bg-purple-50',clr:'text-purple-500',icon:'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z'}
  ].map(s=>`<div class="stat-card fade-in"><div class="stat-icon ${s.bg} ${s.clr}"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="${s.icon}"/></svg></div><div class="stat-value">${s.val}</div><div class="stat-label">${s.label}</div></div>`).join('');
  ...
  if (bg) bg.innerHTML = !bloks?.length
    ? `<div class="col-span-full empty-state py-6"><div class="empty-title">Belum Ada Blok</div></div>`
  ...
  try {
    ...
    document.getElementById('dashTodayCount').textContent = `${cnt} catatan`;
  } catch { const el = document.getElementById('dashAbsenVal'); if (el) el.textContent = '0'; }
}
...
async function loadWbp() {
  ...
  if (tbody) tbody.innerHTML = `<tr><td colspan="8" style="padding:16px"><div style="display:flex;flex-direction:column;gap:10px">${[1,2,3,4].map(()=>`<div style="height:44px;background:#f1f5f9;border-radius:10px;animation:pulse 1.5s infinite"></div>`).join('')}</div></td></tr>`;
  ...
  tbody.innerHTML = data.map((w,i)=>`
    <tr class="fade-in">
      ...
      <td><div style="display:flex;align-items:center;gap:10px"><div class="table-avatar">${w.foto_url?`<img src="${w.foto_url}"/>`:(w.nama?.[0]||'?')}</div><div><div class="font-semibold text-sm">${w.nama}</div><div class="text-xs text-gray-400">${w.asal||'—'}</div></div></div></td>
      ...
      <td><div style="display:flex;gap:6px">
        <button class="btn btn-warning btn-sm btn-icon" onclick="editWbp('${w.id}')">✏️</button>
        <button class="btn btn-danger btn-sm btn-icon" onclick="deleteWbp('${w.id}','${w.nama.replace(/'/g,"\\'")}')">🗑️</button>
      </div></td>
    </tr>`).join('');

  const tot = Math.ceil(count/WBP_PER_PAGE);
  const pag = document.getElementById('wbpPagination');
  pag.innerHTML = tot>1 ? Array.from({length:Math.min(tot,10)},(_,i)=>`<button onclick="wbpPage=${i+1};loadWbp()" class="btn btn-sm ${i+1===wbpPage?'btn-primary':'btn-ghost'}" style="min-width:32px">${i+1}</button>`).join('') : '';
}
...
async function editWbp(id) {
  ...
  v('wbpId',w.id);v('wbpNama',w.nama);v('wbpNoreg',w.no_registrasi);v('wbpJk',w.jk||'L');
  v('wbpBlok',w.blok_id);v('wbpTglMasuk',w.tgl_masuk);v('wbpTglBebas',w.tgl_bebas);
  v('wbpMasa',w.masa_pidana);v('wbpKasus',w.kasus);v('wbpAsal',w.asal);v('wbpCatatan',w.catatan);
  v('wbpStatus',w.status||'aktif');
...
function previewWbpFoto(e) {
  ...
  const r=new FileReader(); r.onload=ev=>{const img=document.getElementById('wbpFotoPreview');img.src=ev.target.result;img.style.display='block';document.getElementById('wbpFotoPlaceholder').style.display='none';}; r.readAsDataURL(wbpFotoFile);
}
...
async function deleteWbp(id,nama){showConfirm('Hapus WBP',`Hapus WBP <strong>${nama}</strong>?`,async()=>{const{error}=await sb.from('wbp').delete().eq('id',id);if(error){showAlert('error','Gagal',error.message);return;}showAlert('success','Dihapus!',`${nama} dihapus`);loadWbp();});}

// ── PEGAWAI ───────────────────────────────────────────────────
async function loadPegawai() {
  const search=document.getElementById('pegawaiSearch')?.value.trim()||'', roleF=document.getElementById('pegawaiRoleFilter')?.value||'';
  const tbody=document.getElementById('pegawaiTableBody');
  if(tbody) tbody.innerHTML=`<tr><td colspan="8" style="padding:16px"><div style="display:flex;flex-direction:column;gap:10px">${[1,2,3].map(()=>`<div style="height:44px;background:#f1f5f9;border-radius:10px;animation:pulse 1.5s infinite"></div>`).join('')}</div></td></tr>`;
  ...
  if(!data?.length){tbody.innerHTML=`<tr><td colspan="8"><div class="empty-state"><div class="empty-title">Belum Ada Pegawai</div><div class="empty-desc">Tambahkan pegawai melalui tombol Tambah Pegawai</div></div></td></tr>`;return;}
  tbody.innerHTML=data.map((p,i)=>`
    <tr class="fade-in">
      <td class="text-gray-400 text-xs">${i+1}</td>
      <td><div style="display:flex;align-items:center;gap:10px"><div class="table-avatar">${p.nama?.[0]||'?'}</div><div><div class="font-semibold text-sm">${p.nama}</div><div class="text-xs text-gray-400">${p.nip||'—'}</div></div></div></td>
      <td class="text-sm">${p.jabatan||'—'}</td><td class="text-sm">${p.pangkat||'—'}</td>
      ...
      <td><div style="display:flex;gap:6px">
        <button class="btn btn-warning btn-sm btn-icon" onclick="editPegawai('${p.id}')">✏️</button>
        ${p.id!==currentAdmin?.id?`<button class="btn btn-danger btn-sm btn-icon" onclick="deletePegawai('${p.id}','${p.nama.replace(/'/g,"\\'")}')">🗑️</button>`:''}
      </div></td>
    </tr>`).join('');
}
function openPegawaiModal(){
  ['pegawaiId','pegawaiUserId','pegawaiNama','pegawaiJabatan','pegawaiPangkat','pegawaiUsername','pegawaiEmail','pegawaiPassword','pegawaiNip'].forEach(id=>{const el=document.getElementById(id);if(el)el.value='';});
...
async function editPegawai(id){
  ...
  const v=(eid,val)=>{const el=document.getElementById(eid);if(el)el.value=val||'';};
  v('pegawaiId',p.id);v('pegawaiUserId',p.user_id);v('pegawaiNama',p.nama);v('pegawaiJabatan',p.jabatan);
  v('pegawaiPangkat',p.pangkat);v('pegawaiUsername',p.username);v('pegawaiEmail',p.email);v('pegawaiNip',p.nip);
  document.getElementById('pegawaiRole').value=p.role||'user'; document.getElementById('pegawaiStatus').value=p.status||'aktif';
  document.getElementById('pegawaiPwdGroup').style.display='none';
}
async function savePegawai(){
  const id=document.getElementById('pegawaiId').value, nama=document.getElementById('pegawaiNama').value.trim(), username=document.getElementById('pegawaiUsername').value.trim(), email=document.getElementById('pegawaiEmail').value.trim(), password=document.getElementById('pegawaiPassword').value;
  if(!nama||!username||!email){showAlert('warning','Perhatian','Nama, username, dan email wajib diisi!');return;}
  if(!id&&(!password||password.length<8)){showAlert('warning','Perhatian','Password minimal 8 karakter!');return;}
  const btn=document.getElementById('pegawaiSaveBtn'); btn.disabled=true; btn.textContent='Menyimpan...';
  try{
    const payload={nama,jabatan:document.getElementById('pegawaiJabatan').value,pangkat:document.getElementById('pegawaiPangkat').value,username,email,nip:document.getElementById('pegawaiNip').value,role:document.getElementById('pegawaiRole').value,status:document.getElementById('pegawaiStatus').value};
    if(id){const{error}=await sb.from('pegawai').update(payload).eq('id',id);if(error)throw error;}
    else{const{data:au,error:ae}=await sb.auth.signUp({email,password});if(ae)throw ae;payload.user_id=au.user.id;const{error}=await sb.from('pegawai').insert(payload);if(error)throw error;}
    showAlert('success','Berhasil!',id?'Data pegawai diperbarui':'Pegawai baru didaftarkan');
    closeModal('pegawaiModal'); loadPegawai();
  }catch(e){showAlert('error','Gagal',e.message);}
  finally{btn.disabled=false;btn.innerHTML='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="w-4 h-4"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"/></svg> Simpan';}
}
async function deletePegawai(id,nama){showConfirm('Hapus Pegawai',`Hapus pegawai <strong>${nama}</strong>?`,async()=>{const{error}=await sb.from('pegawai').delete().eq('id',id);if(error){showAlert('error','Gagal',error.message);return;}showAlert('success','Dihapus!',`${nama} dihapus`);loadPegawai();});}
...
async function loadBlok(){
  ...
  if(tbody) tbody.innerHTML=`<tr><td colspan="7" style="padding:16px"><div style="display:flex;flex-direction:column;gap:10px">${[1,2].map(()=>`<div style="height:44px;background:#f1f5f9;border-radius:10px;animation:pulse 1.5s infinite"></div>`).join('')}</div></td></tr>`;
  ...
  if(!data?.length){tbody.innerHTML=`<tr><td colspan="7"><div class="empty-state"><div class="empty-title">Belum Ada Blok/Kamar</div><div class="empty-desc">Tambahkan blok melalui tombol Tambah Blok</div></div></td></tr>`;return;}
  tbody.innerHTML=data.map((b,i)=>`
    <tr class="fade-in">
      ...
      <td><span class="font-bold text-sm ${(b.wbp?.[0]?.count||0)>=(b.kapasitas||9999)?'text-red-500':'text-green-600'}">${b.wbp?.[0]?.count||0}</span><span class="text-xs text-gray-400"> / ${b.kapasitas||'?'}</span></td>
      <td><div style="display:flex;gap:6px">
        <button class="btn btn-warning btn-sm btn-icon" onclick="editBlok('${b.id}')">✏️</button>
        <button class="btn btn-danger btn-sm btn-icon" onclick="deleteBlok('${b.id}','${b.nama.replace(/'/g,"\\'")}')">🗑️</button>
      </div></td>
    </tr>`).join('');
}
...
async function editBlok(id){
  const{data:b}=await sb.from('blok').select('*').eq('id',id).maybeSingle(); if(!b) return;
  document.getElementById('blokModalTitle').textContent='Edit Blok/Kamar';
  document.getElementById('blokId').value=b.id; document.getElementById('blokNama').value=b.nama;
  document.getElementById('blokKapasitas').value=b.kapasitas||''; document.getElementById('blokJk').value=b.jk||'L';
...
async function deleteBlok(id,nama){showConfirm('Hapus Blok',`Hapus blok <strong>${nama}</strong>?`,async()=>{const{error}=await sb.from('blok').delete().eq('id',id);if(error){showAlert('error','Gagal',error.message);return;}showAlert('success','Dihapus!',`Blok ${nama} dihapus`);loadBlok();});}

// ── RIWAYAT ───────────────────────────────────────────────────
async function loadRiwayatFilters(){
  const bs=document.getElementById('riwayatBlokFilter'); if(bs?.options.length<=1){const{data:bloks}=await sb.from('blok').select('id,nama').order('nama');bloks?.forEach(b=>bs.add(new Option(b.nama,b.id)));}
  const ps=document.getElementById('riwayatPegawaiFilter'); if(ps?.options.length<=1){const{data:pegs}=await sb.from('pegawai').select('id,nama').order('nama');pegs?.forEach(p=>ps.add(new Option(p.nama,p.id)));}
}
async function loadRiwayat(){
  const tbody=document.getElementById('riwayatTableBody');
  if(tbody) tbody.innerHTML=`<tr><td colspan="7" style="padding:16px"><div style="display:flex;flex-direction:column;gap:10px">${[1,2,3,4].map(()=>`<div style="height:44px;background:#f1f5f9;border-radius:10px;animation:pulse 1.5s infinite"></div>`).join('')}</div></td></tr>`;
  try{
    ...
    if(dari)params.append('dari',dari); if(sampai)params.append('sampai',sampai);
    if(blok)params.append('blok_id',blok); if(peg)params.append('pegawai_id',peg);
    ...
    if(!data.length){tbody.innerHTML=`<tr><td colspan="7"><div class="empty-state"><div class="empty-title">Tidak Ada Data</div><div class="empty-desc">Coba ubah filter atau pastikan GAS sudah dikonfigurasi</div></div></td></tr>`;document.getElementById('riwayatPagination').innerHTML='';return;}
    ...
    const tot=Math.ceil(data.length/RIWAYAT_PER_PAGE), pag=document.getElementById('riwayatPagination');
    pag.innerHTML=tot>1?Array.from({length:Math.min(tot,10)},(_,i)=>`<button onclick="riwayatPage=${i+1};loadRiwayat()" class="btn btn-sm ${i+1===riwayatPage?'btn-primary':'btn-ghost'}" style="min-width:32px">${i+1}</button>`).join(''):'';
  }catch{document.getElementById('riwayatTableBody').innerHTML=`<tr><td colspan="7"><div class="empty-state"><div class="empty-title">GAS Belum Dikonfigurasi</div><div class="empty-desc">Atur URL Google Apps Script di Konfigurasi Website</div></div></td></tr>`;}
}
function resetRiwayatFilter(){const today=new Date().toISOString().split('T')[0];document.getElementById('riwayatSearch').value='';document.getElementById('riwayatDari').value=today;document.getElementById('riwayatSampai').value=today;document.getElementById('riwayatBlokFilter').value='';document.getElementById('riwayatPegawaiFilter').value='';riwayatPage=1;loadRiwayat();}
...
async function loadAkun(){
  ...
  const v=(id,val)=>{const el=document.getElementById(id);if(el)el.value=val||'';};
  t('akunNamaDisplay',currentAdmin.nama);t('akunJabatanDisplay',currentAdmin.jabatan);
  t('akunPangkatDisplay',currentAdmin.pangkat);t('akunUsernameDisplay',currentAdmin.username);
  ...
  v('akunNama',currentAdmin.nama);v('akunJabatan',currentAdmin.jabatan);
  const{data:{user}}=await sb.auth.getUser();
  t('akunEmailDisplay',user?.email);v('akunEmail',user?.email);
}
async function saveAkun(){
  const nama=document.getElementById('akunNama')?.value.trim(), jabatan=document.getElementById('akunJabatan')?.value.trim(), email=document.getElementById('akunEmail')?.value.trim();
  if(!nama){showAlert('warning','Perhatian','Nama tidak boleh kosong!');return;}
  try{
    const{error}=await sb.from('pegawai').update({nama,jabatan}).eq('id',currentAdmin.id);if(error)throw error;
    const{data:{user}}=await sb.auth.getUser();
    if(email&&email!==user.email){const{error:e2}=await sb.auth.updateUser({email});if(e2)throw e2;}
