...
// ── SHIFT ────────────────────────────────────────────────────
// Pagi: 00:00–11:59, Siang: 12:00–16:59, Sore: 17:00–23:59 (WIT)
function getShiftNow() {
  const jam = new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jayapura', hour: '2-digit', hour12: false });
  const h = parseInt(jam);
  if (h < 12) return 'Pagi';
  if (h < 17) return 'Siang';
  return 'Sore';
}
const SHIFT_ICON = { Pagi: '🌅', Siang: '☀️', Sore: '🌇' };
const SHIFT_COLOR = { Pagi: '#f59e0b', Siang: '#3b82f6', Sore: '#8b5cf6' };
const SHIFT_BG = { Pagi: '#fef3c7', Siang: '#dbeafe', Sore: '#ede9fe' };
const SHIFT_TC = { Pagi: '#92400e', Siang: '#1e40af', Sore: '#5b21b6' };
...
// ── STATUS ───────────────────────────────────────────────────
const STATUS_ICON  = { 'Di Kamar':'✅','Di Bengkel':'🔧','Di Kebun':'🌿','Di Rumah Sakit':'🏥','Lainnya':'📌' };
const STATUS_BG    = { 'Di Kamar':'#d1fae5','Di Bengkel':'#fef3c7','Di Kebun':'#dcfce7','Di Rumah Sakit':'#dbeafe','Lainnya':'#f1f5f9' };
const STATUS_TC    = { 'Di Kamar':'#065f46','Di Bengkel':'#92400e','Di Kebun':'#14532d','Di Rumah Sakit':'#1e40af','Lainnya':'#475569' };
...
function showAlert(type, title, msg, dur = 4000) {
  ...
  el.innerHTML=`<div style="background:white;border-radius:16px;box-shadow:0 8px 32px rgba(0,0,0,.12);border:1px solid #f1f5f9;overflow:hidden">
    <div style="position:absolute;left:0;top:0;bottom:0;width:4px;background:linear-gradient(${c1},${c2})"></div>
    <div style="display:flex;align-items:flex-start;gap:12px;padding:14px 14px 14px 18px">
      <div style="width:32px;height:32px;border-radius:10px;background:linear-gradient(135deg,${c1},${c2});display:flex;align-items:center;justify-content:center;flex-shrink:0"><svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" style="width:16px;height:16px"><path stroke-linecap="round" stroke-linejoin="round" d="${IC[type]}"/></svg></div>
      <div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:700;color:#1e293b">${title}</div><div style="font-size:12px;color:#64748b;margin-top:2px;line-height:1.5">${msg}</div></div>
      <button onclick="this.closest('.sw-alert').remove()" style="background:none;border:none;cursor:pointer;color:#94a3b8;font-size:18px;padding:0;line-height:1;flex-shrink:0">×</button>
    </div>
    <div style="height:3px;background:#f1f5f9"><div id="swp" style="height:100%;width:100%;background:linear-gradient(90deg,${c1},${c2});transition:width ${dur}ms linear"></div></div>
  </div>`;
...
function showConfirm(title, msg, onOk, type='danger') {
  ...
  ov.innerHTML=`<div id="scc" style="background:white;border-radius:24px;max-width:400px;width:100%;box-shadow:0 24px 64px rgba(0,0,0,.2);transform:scale(.9);opacity:0;transition:all .3s cubic-bezier(.34,1.56,.64,1)">
    <div style="padding:24px 24px 16px"><div style="font-size:16px;font-weight:800;color:#1e293b;margin-bottom:6px">${title}</div><div style="font-size:13px;color:#64748b;line-height:1.5">${msg}</div></div>
    <div style="display:flex;gap:10px;justify-content:flex-end;padding:12px 24px 20px">
      <button id="scc_n" style="padding:9px 18px;border-radius:10px;border:none;background:#f1f5f9;color:#64748b;font-size:13px;font-weight:600;cursor:pointer">Batal</button>
      <button id="scc_y" style="padding:9px 18px;border-radius:10px;border:none;background:${BC[type]||BC.danger};color:white;font-size:13px;font-weight:700;cursor:pointer">Ya, Lanjutkan</button>
    </div>
  </div>`;
...
function emptyState(title, sub='') {
  return `<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;padding:48px 20px;text-align:center">
    ...
    <div style="font-size:14px;font-weight:700;color:#374151">${title}</div>
    ${sub?`<div style="font-size:12px;color:#94a3b8;margin-top:4px;max-width:200px">${sub}</div>`:''}
  </div>`;
}

// Helper: load splash dari site_config
async function loadSplashFromDB(logoWrapId, nameWrapId, descWrapId) {
  try {
    ...
    const sl = document.getElementById(logoWrapId);
    if (sl) {
      sl.style.animation = 'none';
      if (data.logo_url) {
        const img = document.createElement('img'); img.src = data.logo_url;
        img.style.cssText = 'width:100%;height:100%;object-fit:cover';
        sl.appendChild(img);
      } else { sl.textContent = '🏛️'; sl.style.fontSize = '30px'; }
    }
    const sn = document.getElementById(nameWrapId);
    if (sn) { sn.style.cssText = 'color:white;font-size:20px;font-weight:800;background:none;animation:none;height:auto;width:auto;border-radius:0;margin-bottom:5px'; sn.textContent = data.site_name || 'SIMAWAR'; }
    const sd = document.getElementById(descWrapId);
    if (sd) { sd.style.cssText = 'color:rgba(255,255,255,.7);font-size:12px;background:none;animation:none;height:auto;width:auto;border-radius:0;text-align:center'; sd.textContent = (data.site_desc || '').split('\n')[0] || 'Sistem Informasi Monitoring Warga Binaan'; }
  } catch(e) {
