...
// Timezone WIB+2 = WIT (Indonesia Timur = UTC+9)
function todayWIT() {
...
function formatTglWaktu(s) { return formatWIT(s); }
...
function showAlert(type, title, message, duration = 4000) {
  ...
  el.className = 'sw-alert';
  el.style.cssText = 'position:fixed;top:20px;right:20px;z-index:99999;max-width:360px;width:100%;opacity:0;transform:translateX(30px);transition:all 0.35s cubic-bezier(.34,1.56,.64,1)';
  el.innerHTML = `<div style="background:white;border-radius:16px;box-shadow:0 8px 32px rgba(0,0,0,0.12);border:1px solid #f1f5f9;overflow:hidden;position:relative">
    <div style="position:absolute;left:0;top:0;bottom:0;width:4px;background:linear-gradient(${c[type][0]},${c[type][1]})"></div>
    <div style="display:flex;align-items:flex-start;gap:12px;padding:14px 14px 14px 18px">
      <div style="width:32px;height:32px;border-radius:10px;background:linear-gradient(135deg,${c[type][0]},${c[type][1]});display:flex;align-items:center;justify-content:center;flex-shrink:0">
      ...
      <button onclick="this.closest('.sw-alert').remove()" style="background:none;border:none;cursor:pointer;color:#94a3b8;font-size:18px;padding:0;line-height:1">×</button>
    </div>
    <div style="height:3px;background:#f1f5f9"><div id="swprog" style="height:100%;width:100%;background:linear-gradient(90deg,${c[type][0]},${c[type][1]});transition:width ${duration}ms linear"></div></div>
  </div>`;
  document.body.appendChild(el);
  requestAnimationFrame(() => { el.style.opacity='1'; el.style.transform='translateX(0)'; });
  requestAnimationFrame(() => requestAnimationFrame(() => { const p=el.querySelector('#swprog'); if(p) p.style.width='0%'; }));
...
function showConfirm(title, msg, onOk, type='danger') {
  ...
  ov.className = 'sw-confirm';
  ov.style.cssText = 'position:fixed;inset:0;z-index:10000;display:flex;align-items:center;justify-content:center;padding:16px;background:rgba(0,0,0,0.4);backdrop-filter:blur(4px)';
  ov.innerHTML = `<div id="swcbox" style="background:white;border-radius:24px;max-width:400px;width:100%;box-shadow:0 24px 64px rgba(0,0,0,0.2);transform:scale(.9);opacity:0;transition:all .3s cubic-bezier(.34,1.56,.64,1)">
    ...
    <div style="display:flex;gap:10px;justify-content:flex-end;padding:12px 24px 20px">
      <button id="swcancel" style="padding:9px 18px;border-radius:10px;border:none;background:#f1f5f9;color:#64748b;font-size:13px;font-weight:600;cursor:pointer">Batal</button>
      <button id="swok" style="padding:9px 18px;border-radius:10px;border:none;background:${bc[type]||bc.danger};color:white;font-size:13px;font-weight:700;cursor:pointer">Ya, Lanjutkan</button>
    </div>
  </div>`;
  document.body.appendChild(ov);
  const box = ov.querySelector('#swcbox');
  requestAnimationFrame(() => { box.style.transform='scale(1)'; box.style.opacity='1'; });
  ov.querySelector('#swcancel').onclick = () => ov.remove();
  ov.querySelector('#swok').onclick = () => { ov.remove(); onOk(); };
...
st.textContent = '@keyframes swpulse{0%,100%{opacity:1}50%{opacity:.5}}';
